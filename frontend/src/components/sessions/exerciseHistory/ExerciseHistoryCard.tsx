    ...
    <div className="bg-white rounded-2xl p-4 shadow-sm">
        ...
        {recentEntries.map((entry, idx) => (
          ...
          >
            <span className="font-semibold">
              {entry.sessionExercise.repetitions}
            </span>{" "}
            rep |{" "}
            <span className="font-semibold">{entry.sessionExercise.sets}</span>{" "}
            set |{" "}
            <span className="font-semibold">
      ...
      {lastEntry && (
        <p className="text-xs text-gray-500">
          Last: {lastEntry.sessionExercise.repetitions} ×{" "}
          {lastEntry.sessionExercise.sets} @ {lastEntry.sessionExercise.weight}{" "}
          kg
