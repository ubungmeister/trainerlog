...
const addEntrySchema = z.object({
  ...
  repetitions: z.number({ invalid_type_error: "Required" }).min(1, "Must be at least 1"),
  sets: z.number({ invalid_type_error: "Required" }).min(1, "Must be at least 1"),
  weight: z.number({ invalid_type_error: "Required" }).positive("Must be at least 1"),
});
...
export function AddEntryModal({
  ...
  const allExercises = useMemo(() => {
    const clientExIds = new Set(
      clientExercises.filter((ce) => ce.activeClientExercise).map((ce) => ce.exerciseId),
    );
    ...
    const fromShared = exercises
      .filter((e) => e.sharedExercise && e.activeExercise && e.id && !clientExIds.has(e.id))
  ...
  const onSubmit = (data: AddEntryFormValues) => {
    const normalizedDate = new Date(data.date).toISOString().slice(0, 10);
    ...
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-end justify-center z-50">
      <div className="bg-white w-full max-w-lg rounded-t-2xl p-5 pb-24 max-h-[85vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-lg font-bold text-gray-900">Add Exercise Entry</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="mb-4">
            ...
            {errors.exerciseId && (
              <p className="text-red-500 text-xs mt-1">{errors.exerciseId.message}</p>
            )}

            {isExerciseListOpen && (
              <div className="mt-2 border border-gray-200 rounded-xl overflow-y-auto max-h-40 bg-white shadow-sm">
                {allExercises.length === 0 ? (
                  <div className="px-4 py-3 text-sm text-gray-500">No exercises available</div>
                ) : (
                  allExercises.map((ex) => (
                    <button
                      key={ex.id}
                    ...
                    >
                      {ex.name}
                      {exerciseId === ex.id && <Check size={18} className="text-primary-bg" />}
                    </button>
                  ))
                )}
              </div>
          ...
          <div className="grid grid-cols-3 gap-3 mb-4">
            <div>
              ...
              />
              {errors.repetitions && (
                <p className="text-red-500 text-xs mt-1">{errors.repetitions.message}</p>
              )}
            </div>
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-3 border border-gray-200 rounded-xl text-base text-center focus:outline-none focus:ring-2 focus:ring-primary-bg/30"
              />
              {errors.sets && <p className="text-red-500 text-xs mt-1">{errors.sets.message}</p>}
            </div>
            <div>
